...
router.get('/HotProducts/:numberOfProducts', function (req, res) {
    var numberOfProducts = req.params.numberOfProducts;
...
router.get('/RecomandedProducts/:userName/:numberOfProducts', function (req, res) {
    ...
    DButilsAzure.Select("Select Id from Users Where UserName='"+userName+"'").then(function (result) {
        if (result.length > 0) {
           ...
           DButilsAzure.Select("Select CategoryId FROM UserCategories Where UserId='"+userId+"'").then(function (result) {
            if (result.length > 0) {
                var isFirst= true;
                var query= "SELECT ProductId FROM ProductCategories WHERE"
                ...
                res.send({"status": "OK", "Message" : "canot find RecomandedProducts for this user, the list is empty", "resObject":{"Products" : []} });
    ...
    res.send({"status": "OK", "Message" : "user canot be found" });
    }).catch(function(err) {
        res.send({"status": "ERROR", "Message": err});
    });
});
...
router.get('/CategoryProductsById/:categoryId', function(req,res){
    var categoryId = req.params.categoryId;
    DButilsAzure.Select("Select * from ProductCategories JOIN Products ON ProductCategories.ProductId=Products.Id WHERE ProductCategories.CategoryId='"+categoryId+"'").then(function (result) {
        res.send({"status": "OK", "Message":"successfully get Category Products","resObject":{ "Products" : result } });
    }).catch(function(err) {
        res.send({"status": "ERROR", "Message": err});
    });
});
